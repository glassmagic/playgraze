...
- **Persistence**: `meta` object ↔ `localStorage["graze_save"]` (best, cores, owned weapons/hulls/pilots, consumable `droneStock`, `seen` weapon discovery, pilot, scheme, callsign `player`, shop levels, daily streak).
...
- **Hangar Shop tabs**: WEAPONS / DRONES / SHIPS / PILOTS / SHIP SYSTEMS / PAINTWORK. Drones are expensive consumables: every purchase adds one unit to that type's persistent bay inventory, number/numpad keys `1`–`8` deploy the eight fixed types, the HUD slots are tappable on mobile, and any mix/quantity can be active simultaneously. Each drone's integrity drains across a type-specific natural lifespan and faster under contact, bullets, mortars, meteors, and laser sweeps. Their overlapping broadcasts redirect nearby non-boss enemies, while the roster covers ballistic, arc, laser, mortar, flak, gravity, repair, and decoy roles. Hull `size` scales drawn ship, heart ring, heat gauge **and hitbox** (`player.r`). Pilots (free) set aim (spread × acc + per-shot jitter, floor at acc 0.65) and fire-rate multiplier.
