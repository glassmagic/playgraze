...
- **Hangar Shop tabs**: WEAPONS / DRONES / SHIPS / PILOTS / SHIP SYSTEMS / PAINTWORK. Drones are expensive permanent blueprints with one equipped deployment per run (`E` or the HUD control); their integrity drains across a type-specific natural lifespan and faster under contact, bullets, mortars, meteors, and laser sweeps. Their broadcast redirects nearby non-boss enemies, while eight types cover ballistic, arc, laser, mortar, flak, gravity, repair, and decoy roles. Hull `size` scales drawn ship, heart ring, heat gauge **and hitbox** (`player.r`). Pilots (free) set aim (spread × acc + per-shot jitter, floor at acc 0.65) and fire-rate multiplier.
