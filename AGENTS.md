...
- **Persistence**: `meta` object ↔ `localStorage["graze_save"]` (best, cores, owned weapons/hulls/pilots, permanent `droneResearch`, consumable `droneStock`, named-boss completion in `bossKills`, `seen` weapon discovery, pilot, scheme, callsign `player`, shop levels, daily streak).
...
- **Enemies**: `ETYPES` + `WSPAWN`; per-run shuffled introductions (`rollIntroPlan`: chaser/drifter/weaver fixed at 0/25/55s, rest randomized on 30s slots from 85s). Elites (after 2:00) carry affixes: shield / split / spray. Special: `courier` (fleeing treasure, excluded from rotation), `warden` (directional bullet-blocking shield — heavy weapons bypass), `bomber` (mines via `zones`), `leech` (drains surge through a tether). After 10:00, every additional completed minute adds another 10% health and damage to all enemies; current enemies rescale at the boundary, future spawns/bosses inherit the current multiplier, and fractional damage accumulates against the integer hull meter.
- **Bosses**: THE HERALD (easier, once, at 1:30, generic ring/burst/charge/summon sampler), first named boss at 3:00, then a new one every 120s, cycling four signature kits in a per-run shuffled order (`G.bossOrder`) (`BOSS_KITS`): VOIDMAW (drops soft wells, vacuum charges), HEXAGON PRIME (anchors at center, rotating spiral arms), NULL TYRANT (telegraphed triple dashes with bullet wake), THE CANNIBAL (inhales nearby enemies — flagged `eaten`, removed scoreless — and spits them back as fast shots). Named-boss victories persist by kit in `meta.bossKills`; defeating all four kinds at least once unlocks drone mode (THE HERALD does not count). Repeat visits differ by scaling only (hp × bossN × time). The spawn "tide" shapes fights: regular/elite/courier spawns halve in the 30s before any boss is due, stop entirely in the final 5s, and run at 2× for 10s after a boss dies (`G.rushT`). Letterbox intro + kill-cam via the generic `G.cine` slow-mo/zoom system. A slain boss has a 1-in-3 chance of leaving a pilot prisoner (drawn twice life-size, waving, repelling nearby shards/enemies and drifting toward arena center); collecting one unlocks a random undiscovered pilot.
...
- **Weapons**: all fire logic branches in `updatePlayer`'s auto-fire; waves/flail/holes have their own update fns. The starting PULSE ARRAY's primary shots deal 70% of the shared base damage; other weapons retain their existing multipliers. Heavy weapons (arc/beam/rift/flail) scale with `heavyBoost` (pierce/ricochet/homing each +12%). The CONRADIATOR uses `player.heat`: each shot adds 2.5% heat; its strongly nonlinear spool runs 2→15/s through 65% heat in roughly three seconds, then kicks to 34/s. Primary bullets deal 22% base damage, full heat vents back to a true 0-heat/2-shots-per-second cold restart, retargeting loses 60% heat, and cooling begins after 1.1s off-trigger so the opening shots can retain heat.
...
- **Hangar Shop tabs**: WEAPONS / DRONES / SHIPS / PILOTS / SHIP SYSTEMS / PAINTWORK. Before all four named boss kinds have been defeated, DRONES shows persistent `N/4` progress and a boss checklist but no inventory or in-run HUD. Once unlocked, each of the eight drone designs has a one-time 6,000–100,000-core research purchase; subsequent expendable builds cost exactly one tenth of research and add one unit to persistent `droneStock`. Existing v1.19 purchases are grandfathered into `droneResearch`, even when their stock has already reached zero. Number/numpad keys `1`–`8` deploy units, HUD slots are tappable on mobile, and any mix/quantity can be active simultaneously. Higher research tiers provide progressively longer field life, integrity, and stronger utility. Each drone's integrity drains naturally and faster under contact, bullets, mortars, meteors, and laser sweeps. Their overlapping broadcasts redirect nearby non-boss enemies, while the roster covers ballistic fire, chain lightning, piercing laser, area mortar, projectile interception, gravity control, hull repair/repulsion, and a maximum-attraction tank/decoy. Hull `size` scales drawn ship, heart ring, heat gauge **and hitbox** (`player.r`). Pilots (free) set aim (spread × acc + per-shot jitter, floor at acc 0.65) and fire-rate multiplier.
...
- **Performance**: projectile homing and collision always use the original straight-line scans (faster than bucket traversal at measured densities). Below 256 enemies, other targeting/effect queries do the same; at 256+, those less frequent queries use a 160px spatial grid rebuilt before and after enemy movement. Every candidate still gets the original exact distance test. Hot transient objects (bullets, shards, particles, texts, bolts, flares, Death Bloom rings) use fixed-shape pools plus stable compaction instead of per-hit `splice`. Glow/RGB/path/audio-noise data and HUD nodes are cached. `perf-fixture.html` is the repeatable browser stress/parity check; compare its timings only on the same browser and hardware.
...
- **Base ship speed was deliberately lowered three times** (235→185→155→46.5) so hull choice, thruster upgrades, and speed cards carry much more of the movement curve. Don't "fix" the very slow ship.
