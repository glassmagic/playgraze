...
- **Persistence**: `meta` object ↔ `localStorage["graze_save"]` (best, cores, owned weapons/hulls/pilots, consumable `droneStock`, named-boss completion in `bossKills`, `seen` weapon discovery, pilot, scheme, callsign `player`, shop levels, daily streak).
...
- **Hangar Shop tabs**: WEAPONS / DRONES / SHIPS / PILOTS / SHIP SYSTEMS / PAINTWORK. Before all four named boss kinds have been defeated, DRONES shows persistent `N/4` progress and a boss checklist but no inventory or in-run HUD. Once unlocked, drones are expensive consumables spanning a 2,000–32,000-core ladder: every purchase adds one unit to that type's persistent bay inventory, number/numpad keys `1`–`8` deploy the eight fixed types, the HUD slots are tappable on mobile, and any mix/quantity can be active simultaneously. Each drone's integrity drains across a type-specific natural lifespan and faster under contact, bullets, mortars, meteors, and laser sweeps. Their overlapping broadcasts redirect nearby non-boss enemies, while the deliberately broad roster covers cheap ballistic fire, chain lightning, piercing laser, area mortar, projectile interception, gravity control, hull repair/repulsion, and a maximum-attraction tank/decoy. Hull `size` scales drawn ship, heart ring, heat gauge **and hitbox** (`player.r`). Pilots (free) set aim (spread × acc + per-shot jitter, floor at acc 0.65) and fire-rate multiplier.
